...
def MultiIndex():
    ...
    # return (cityListIndex, regionListIndex, streetListIndex, communityListIndex)
...
#         df['cAa'].iloc[i] = pointNum
    ...
    df_new = pd.concat([df['perPrice'], df1, df['houseSize'], df21, df['decorate'], df['houseAge'], df[
        'SingelHouse'], df007, df4,
                        ],
                       axis=1)
    df_new.to_csv("myClean2.csv", columns=df_new.iloc[0].keys())
    # print(df_new)
    # printField(df_new)
    # cityList = []
    # regionList = []
